...
Recuerda: Tu trabajo NO es resolver el problema, sino ENTENDERLO y TRANSFERIR correctamente.`;
  ...
  const verbs = [
    { verb: 'pause', length: 0.5 },
    {
      ...
      eventHook: `${process.env.BASE_URL}/event`,
      toolHook: `${process.env.BASE_URL}/tool-hook`,
      llmOptions: {
        systemPrompt,
      ...
      tools: [
        {
          ...
          description: 'Transfiere la llamada al IVR de Banorte cuando detectas la intención del usuario. Usa esta función SOLO cuando tengas claro qué necesita el cliente. Después de llamar esta función, despídete brevemente del usuario.',
          parameters: {
            type: 'object',
            properties: {
              intent: {
                type: 'string',
                description: 'La intención detectada del usuario. Debe ser uno de los valores válidos.',
                enum: INTENT_NAMES,
              },
            },
            required: ['intent'],
