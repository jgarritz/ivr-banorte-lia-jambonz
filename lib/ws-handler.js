...
const SYSTEM_PROMPT = `Eres LiA, asistente virtual de Banorte. Tu ÚNICA función es entender qué necesita el cliente y llamar a la función transfer_to_ivr.

⚠️ REGLA ABSOLUTA: Para transferir al cliente DEBES INVOCAR LA FUNCIÓN transfer_to_ivr. Solo decir verbalmente que vas a transferir NO sirve. La transferencia solo ocurre si ejecutas la función. Sin excepción.

FLUJO OBLIGATORIO:
1. Saluda brevemente: "Hola, soy LiA de Banorte. ¿En qué te puedo ayudar?"
2. Escucha la necesidad del cliente.
3. Si entiendes la intención: EJECUTA INMEDIATAMENTE transfer_to_ivr con el intent correcto.
4. Si no entiendes: haz UNA sola pregunta clarificadora, luego EJECUTA transfer_to_ivr.
5. Si la intención no encaja con ninguna opción específica: usa "saldos_servicios".

DESPUÉS DE LLAMAR transfer_to_ivr:
- Di ÚNICAMENTE: "Un momento, te estoy transfiriendo."
- Inmediatamente llama a hangUp para completar la transferencia.
- NO digas "hasta luego", NO te despidas, NO digas nada más.

REGLAS:
- Sé muy breve. Máximo 1-2 oraciones por turno.
- Habla en español mexicano.
- NUNCA te despidas antes de transferir. La transferencia ocurre cuando llamas hangUp.
...
function buildLlmVerb(callSid) {
  return {
    ...
    toolHook: `${process.env.BASE_URL}/tool/transfer_to_ivr?callSid=${callSid}`,
    llmOptions: {
      systemPrompt: SYSTEM_PROMPT,
      voice: '7b1ea86f-5621-48bb-bf3c-607568ec3a15',
      firstSpeaker: 'FIRST_SPEAKER_AGENT',
      selectedTools: [
        {
          temporaryTool: {
            modelToolName: 'transfer_to_ivr',
            description: 'Inicia la transferencia al IVR de Banorte. Llama a esta función cuando entiendas la intención del cliente. Después de llamarla, di "Un momento, te estoy transfiriendo." y llama a hangUp inmediatamente.',
            dynamicParameters: [
              {
                name: 'intent',
...
function createWsServer(httpServer) {
  ...
  wss.on('connection', (ws, req) => {
    ...
    ws.on('message', (rawData) => {
      ...
      logger.info({ type: msg.type, msgid: msg.msgid }, 'Mensaje recibido');
      ...
      switch (msg.type) {
        case 'session:new': {
          ...
          const ack = {
            ...
            data: [
              ...
              buildLlmVerb(callState.callSid),
            ],
          };
          logger.info({ msgid: msg.msgid }, 'Enviando ack con LLM Ultravox');
        ...
        default:
          logger.info({ type: msg.type, fullMsg: JSON.stringify(msg) }, 'Mensaje NO manejado - raw completo');
      }
    });
